...
public class phase1step4R2V0 { // R2 is a comparison between the mean grade and the predicted grade by the model, the closer to 1 the better the model is see 
//also known as coefficient of determination or R squared
    ...
    System.out.println("Building best decision stump for each course...");
        ...
        for (int c = 0; c < courseCount; c++) {
            allStumps[c] = findBestDescisionStumpForCourse(c); //finding the best stump to reduce randomness
        }
    ...
    ArrayList<Double>[] TopR2sByCourse = new ArrayList[courseCount];
        ...
        ArrayList<Double> truegrades = CurrentGradesModel.getAllValidGradesCourse(courseId); // Get all grades
    ...
    int sampleSize = truegrades.size(); // choose  up to 2000 to have a mix of accuracy and computation time 
    
    for (int i = 0; i < sampleSize; i++) {
        int studentId = CurrentGradesModel.getAllStudentIdsOfCourseWithGrade(courseId).get(i);
    ...
    double[] truegrade = new double[sampleSize];
            ...
            for (int i = 0; i < sampleSize; i++) {
                ...
                truegrade[i] = CurrentGradesModel.getGrade(studentId, courseId);
            }
    ...
    // Helper class to store forest and its R^2 value
    private static class ForestsR2 {
        ...
        ForestsR2(int[] forest, double r2) { this.forest = forest; this.r2 = r2; }
    }
    ...
    public static double calculateR2(double[] truegrade, double[] forestsPred) {
        ...
        double r2 = 1.0 - (ssRes / ssTot); // R^2 formula
        if (r2 > 0.9) { return -1.0;} //penalizing overfitting
        else { return r2; }
    }
    ...
    private static Feature findBestPropertyToGuessGrade(int courseId) {
        ...
        return new NumericalFeature(30, 0.0);
    }
